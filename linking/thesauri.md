...
- [Thesauri](#thesauri)
  ...
  - [Editing Thesauri](#editing-thesauri)
    - [Editing in Cadmus UI](#editing-in-cadmus-ui)
    - [Editing in Cadmus CLI](#editing-in-cadmus-cli)
    - [Editing Manually](#editing-manually)
...
A thesaurus is a list of predefined terms, each representing an entry in that list. Each **entry** in a thesaurus object has:

- an arbitrary `id`, which must be unique in the context of each thesaurus;
- a text `value` to be displayed to end users.

For instance, you might have a list of languages to pick from, like English, French, Italian, etc.: in this case, your thesaurus would have its own ID, e.g. languages, plus a number of entries like `eng`=`English`, `fre`=`French`, `ita`=`Italian`, etc.
...
All thesauri are initially defined in the profile **JSON document** with the configuration for your project. On first startup, when seeding the initial database, the Cadmus API service will read thesauri from this JSON document and inject them into the corresponding collection in the database.

In its JSON form, typically used to seed thesauri in a project, the list always is a flat list (an array). Yet, using conventions you can define two types of lists:

- **flat**: a plain list of entries.
- **hierarchical**: a tree-like list of entries, with parent and children.

## Editing Thesauri

Typically you predefine all your thesauri in the project's seed profile. Once seeded, the profile JSON document will no longer be used (unless you remove the corresponding database so that it needs to be seeded again). Users will be able to change thesauri from the editor, using the Cadmus CLI tool, or by directly updating the database.

>If you change your thesauri in the database it is good practice to synch the profile JSON document with them. The easiest way to do it is exporting the thesauri into JSON files and replacing them in the profile document.

### Editing in Cadmus UI

To change thesauri directly from the editor you need an administrator account. In this case, Cadmus provides:

- a **thesaurus editor**. Note that this can't be used for the `models` thesaurus.
- a **thesaurus importer**.
...
The **import** methods provided in the Cadmus editor may adopt 3 different behaviors:
...
Cadmus stores thesauri in its MongoDB database, so you can also directly modify them. This is recommended only if you are going to automate this process with some complex logic which does not allow you to use the CLI tool or other methods.

1. export the `thesauri` collection from an up-to-date database. This can be created by launching the editor with updated thesauri in an environment where there is no database for that project. In this case, the databases will be created anew, together with their thesauri. The fastest export type is probably a gzipped archive resulting in a single compressed file, like e.g. `thesauri.agz`.
